...
class ConvolutionalNN(NeuralNetworkBase):
    ...
    def construct_neural_network(self, input_size=1000):
        ...
        # Create the model
        self.input_tensor = tf.placeholder(tf.float32, [None, 512])
    ...
    def train_neural_network(self, samples, labels, samples_test, labels_test):
        ...
        print("Optimization Finished!")
            ...
            #
            # if epoch%100 == 0:
            #     train_accuracy = self.accuracy.eval(feed_dict={self.input_tensor: batch_xs, self.output_tensor: batch_ys, self.keep_prob: 1.0})
            #     print("step %d, training accuracy %.7f"%(epoch, train_accuracy))
            # self.train_step.run(feed_dict={self.input_tensor: batch_xs, self.output_tensor: batch_ys, self.keep_prob: 0.5})
        ...
        #
        #     crctPreds = np.append(crctPreds, np.matrix(self.correct_prediction.eval(feed_dict={self.input_tensor:batch_xs, self.output_tensor: batch_ys, self.keep_prob: 1.0})))
        #     print("Prediction size: ", crctPreds.shape)
        #
        # crctPreds.astype(int)
        # print()
        # print("crctAccuracy: ",np.mean(crctPreds))
    ...
    def test_accuracy_of_solution(self, samples, labels, samples_test, labels_test, reshape=True):
        ...
        #     crctPreds = np.append(crctPreds, np.matrix(correct_predictions.eval(feed_dict={self.input_tensor:samples_test, self.output_tensor: labels_test, self.keep_prob: 1.0})))
        # print("Prediction size: ", crctPreds.shape)
        #
        # crctPreds.astype(int)
        # print()
